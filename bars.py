...
def get_biggest_bar(data):
    ...
    while i < len(data):
        if data[i]["Cells"]["SeatsCount"] == data[
                max_seats]["Cells"]["SeatsCount"]:
            sp_max.append(data[i]["Cells"]["Name"])
        elif data[i]["Cells"]["SeatsCount"] > data[max_seats][
                "Cells"]["SeatsCount"]:
            max_seats = i
            sp_max = []
            # нашли больший бар, очищаем список больших баров
            sp_max.append(data[i]["Cells"]["Name"])
        i = i + 1
    return sp_max
...
def get_smallest_bar(data):
    ...
    while i < len(data):
        if data[i]["Cells"]["SeatsCount"] == data[
                min_seats]["Cells"]["SeatsCount"]:
            sp_min.append(data[i]["Cells"]["Name"])
        elif data[i]["Cells"]["SeatsCount"] < data[min_seats][
                "Cells"]["SeatsCount"]:
            min_seats = i
            sp_min = []
            # нашли меньший бар, очищаем список малых баров
            sp_min.append(data[i]["Cells"]["Name"])
        i = i + 1
    return sp_min
...
def get_closest_bar(data, longitude, latitude):
    i = 0
    min_dlina = math.sqrt((float(data[0]["Cells"]["geoData"]["coordinates"][
                          0]) - longitude)**2 + (float(data[0]["Cells"][
                              "geoData"]["coordinates"][1]) - latitude)**2)
    sp_dl = []

    while i < len(data):
        dlina = math.sqrt((float(data[i]["Cells"]["geoData"]["coordinates"][
                          0]) - longitude)**2 + (float(data[i]["Cells"][
                              "geoData"]["coordinates"][1]) - latitude)**2)
        if min_dlina == dlina:
            ...
            min_dlina = dlina
            sp_dl = []
            # нашли ближайший бар, очищаем список близких баров
            sp_dl.append(data[i]["Cells"]["Name"])
        i = i + 1
    return sp_dl
...
if __name__ == '__main__':
    ...
    x_gps = input(
        "Пожалуйста, введите координаты широты вашего местоположения: ")
    while not is_digit(x_gps):
        x_gps = input("Широта введена неверно, повторите ввод: ")
    x_gps = float(x_gps.replace(',', '.'))

    y_gps = input(
        "Пожалуйста, введите координаты долготы вашего местоположения: ")
